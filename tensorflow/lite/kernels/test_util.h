...
// FloatEq() in FP32 mode; Fp16Eq() in FP16 mode, i.e. it checks if the actual
// number almost equals the expected number with the tolerance of 4 FP16 ULPs.
::testing::Matcher<std::tuple<float, float>> FloatingPointAlmostEq();

// Eq() in FP32 mode; Fp16Eq() in FP16 mode, i.e. it checks if the actual number
// almost equals the expected number with the tolerance of 4 FP16 ULPs.
